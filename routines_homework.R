...
kfoldCV.bayes <- function(y,x,K=10,seed) {
  ## Perform K-fold cross-validation for bayes regression estimate suing mombf modelselection function
  ## Input
  ## - y: response
  ...
  if (ncol(x)>0) {
    for (k in 1:K) {
      sel <- subset==k
      fit <- modelSelection(y=y, x=x, priorCoef=zellnerprior(taustd=1), priorDelta=modelbbprior(1,1))
