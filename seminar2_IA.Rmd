...
```{css}
#Intercept converges a bit above 0, while GRE converges around 0.25 with only 1000 iterations
...
```{css}
#Intercept and rank 2,3 are the quickest to converge, but overall we need about 3300 iterations to converge in all covariates
#the variance within and between chains seems consistent iwth all covariates
...
```{css}
#Estimations are very similar, results are not surprising as the default bayesian is no providing any additional information than MLE OLS
...
The function `postProb` outputs models posterior probabilities $p(\gamma|\mathbf{y})$ where the column `modelid` lists the variables included in each model. We show the top 10 models.
